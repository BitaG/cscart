...
class RusCustomerGeolocation
{
    ...
    /**
     * Provides condition for predefined cities list manipulation.
     *
     * @return string
     */
     ...
     * @return array
     */
    protected function getSchema($schema_name)
    {
        return fn_get_schema('rus_customer_geolocation', $schema_name);
    }

    /**
     * Checks if all location fields are empty.
     *
     * @param array $location Location to check
     *
     * @return bool
     */
    public function isLocationEmpty(array $location)
    {
        foreach ($this->getAddressFields() as $field) {
            if ($this->getLocationField($location, $field) !== '') {
                return false;
            }
        }
