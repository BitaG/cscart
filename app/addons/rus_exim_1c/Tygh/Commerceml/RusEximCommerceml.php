...
class RusEximCommerceml
{
    ...
    public function importProductOffersFile($data_offers, $import_params)
    {
        ...
        foreach ($data_offers -> {$cml['offers']} -> {$cml['offer']} as $offer) {
            ...
            if (isset($offer -> {$cml['bar']}) && $this->s_commerceml['exim_1c_import_product_code'] == 'bar') {
                $product['product_code'] = (string) $offer -> {$cml['bar']};
            } elseif (isset($offer -> {$cml['article']}) && $this->s_commerceml['exim_1c_import_product_code'] == 'article') {
                $product['product_code'] = (string) $offer -> {$cml['article']};
            }
            ...
            if (empty($combination_id)) {
                ...
                if ($schema_version == '2.07') {
                    ...
                    $options = $this->addProductCombinations($offer, $product_id, $import_params, $combination_id, $variant_data, isset($product['product_code']) ? $product['product_code'] : false);
    ...
    public function updateProductCombinations($offer, $product_id, $combination_id, $options, $import_params)
    {
        ...
        if (isset($offer->{$this->cml['bar']}) && $this->s_commerceml['exim_1c_import_product_code'] == 'bar') {
            $article = (string) $offer->{$this->cml['bar']};
        } else {
